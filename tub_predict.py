...
def main():
    ...
    # frames, with their pseudo-label mask when we have one
    pairs = []
    if args.data is not None:
        listing = args.data / "seg_label" / "list" / f"{args.split}_gt.txt"
        rows = [l.split() for l in listing.read_text().splitlines() if l.strip()]
        step = args.stride or max(1, len(rows) // args.num)
        for r in rows[::step][:args.num]:
            pairs.append((args.data / r[0][1:], args.data / r[1][1:]))
        print(f"{len(pairs)} frames from the {args.split} split ({len(rows)} available)")
    else:
        frames = sorted(args.tub.glob("*.jpg"))
        step = args.stride or max(1, len(frames) // args.num)
        pairs = [(p, None) for p in frames[::step][:args.num]]
